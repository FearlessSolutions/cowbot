...
var TICKET_COST = 18.0;
var TICKET_ALERT_THRESHOLD = 5;
var ALERT_USER = "@sshep";
...
casper.thenOpen(newValidationLink, function(){
  ...
  if(skidata_message.match(current_balance_matcher)){
    ...
    if(current_balance < threshold_balance){
      console.log("CRITICAL: Insufficient funds for validation!");
    } else {
      var remaining_tickets = Math.ceil((current_balance - threshold_balance)/TICKET_COST);
      if (remaining_tickets <= TICKET_ALERT_THRESHOLD){
        console.log("WARNING: only " + remaining_tickets + " full-price validations remain. " + ALERT_USER);
      }
    }
  } else {
