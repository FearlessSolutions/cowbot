...
module.exports = (robot) ->
  ...
  robot.respond /validate ([0-9]+)$/i, (msg) ->
    #TODO: Replace unless
    unless msg.message.room is "C2LRCM4DN"
      ticket = msg.match[1]
      if ticket.length isnt 6
        ...
        user = msg.message.user.name.toLowerCase()
        #command = "curl -s \"http://parking-service:3000/?first_name=cowbot&last_name=#{user}&ticket=#{ticket}\""
        ...
        #console.log "Calling external command: #{command}"
        #@exec command, (error, stdout, stderr) ->
        #  if error is null
        #    msg.send stdout unless stdout is null
        #    msg.send stderr unless stderr is null
        #    upload_picture(robot, msg, "./response.png", ticket, "Validation Result for ticket #{ticket}", "validation-#{ticket}.png")
        #  else
        #    msg.send "Error!"
        #    upload_picture(robot, msg, "./response.png", ticket, "Error Message for ticket #{ticket}", "error-#{ticket}.png")
